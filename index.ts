...
function conversions(userAns: { from: string; to: string; amount: number }) {
...
// Function for displaying table
function displayTable(userAns: { amount: any; from: string; to: string }) {
  const convertedAmount = conversions(userAns);
  const table = new Table
  table.push(
    { [chalk.bgBlue('Basic Conversion')]: `1 ${userAns.from} = ${currency[userAns.to]} ${userAns.to}` },
    { [chalk.bgBlue('Converted Amount')]: `${userAns.amount} ${userAns.from} = ${convertedAmount.toFixed(2)} ${userAns.to}` }
  );
